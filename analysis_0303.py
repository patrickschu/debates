...
# moving parts
dir="G:/debate/debatefiles/"
outputfilename="debates_0307"
print "start"

#getting files etc
files=[f for f in os.listdir(dir) if not f.startswith(".")]
print "files we're working with: {}".format(",".join(files))
annotations=['CROSSTALK', 'LAUGHTER', 'APPLAUSE', 'CHEERS', 'CHEERING', 'INAUDIBLE']
...
for fili in files:
        ...
        words=[w for w in nltk.word_tokenize(inputfile) if not re.match("\W+", w) and w not in annotations]
        sentprep=re.sub("[\n+]", " ", inputfile)        
